...
const Initialize = async () => {
    ...
    const posBufferSize = 5 * 4 * 4;  // позиция вершин - 5 32-битных числа с плавающей запятой
                                      // (по 4 байта каждое) для четырёх вершин
    const indexBufferSize = 6 * 4;  // индексы вершин - 6 индексов (по 4 байта каждое) нужно что бы
                                    // нарисовать квадрат
    ...
    const indexValues = new Uint32Array(indexBufferSize / 4);

    // устанавливаем индексы вершин
    indexValues.set([0, 1, 2, 1, 2, 3], 0);
    // устанавливаем позиции вершин
    posValues.set([-0.5, 0.5, 0.1, 0.2, 0.3], 0);
    posValues.set([0.5, 0.5, 0.1, 0.2, 0.3], 5);
    posValues.set([-0.5, -0.5, 0.7, 0.8, 0.9], 10);
    posValues.set([0.5, -0.5, 0.7, 0.8, 0.9], 15);
    ...
    function render() {
        ...
        // 6 - столько нужно вершин, чтобы нарисовать квадрат
        renderPassEncoder.drawIndexed(6, countRect);
